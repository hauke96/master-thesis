...
Current routing approaches are either not capable of correctly connecting arbitrary locations (in the case of graph-based algorithm like A*) or do not allow the use of speedup techniques (for example in the \term*[continuous Dijkstra paradigm]{continuous Dijkstra} approach).
...
\label{sec:design-decisions}
	...
	\subsection{Early implementation based on the continuous Dijkstra paradigm}
		...
		Early stages of the continuous Dijkstra approach used a very naive and simple implementation without optimizations mentioned in recent literature on this topic.
...
\label{sec:components}
	...
	A third but internal component, which is not intended to be used from outside the algorithm, is the \texttt{VisibilityGraphGenerator}.
	...
	In an agent-based simulation, each agent has the ability to make its own decisions, which includes the calculation of routes as part of its path planning routine.
	...
	\Cref{fig:components-sequence-routing} illustrates the internal tasks of a routing request using the \texttt{OptimalPath(source, destination, weightFunction)} function.
...
%	For convenience and due to required preparations, routing queries are put to the hybrid visibility graph.
	To fulfill the overall concept of this work, finding optimal paths between arbitrary locations, is must be ensures that these location are reachable via the hybrid visibility graph.
	...
	To perform this task, the hybrid visibility graph uses the \texttt{VisibilityGraphGenerator} to determine visibility neighbors of a single vertex by calling the same method as the graph generation does.
