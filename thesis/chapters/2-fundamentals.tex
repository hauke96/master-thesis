...
%			In each of these graphs one sub-query is performed, which eventually meet in the same vertex.
		...
		\label{subsubsec:other-speedup-methods}
			...
			There are numerous other speedup methods, such as \term{TRANSIT} and the \term{hub label} strategy.
			
			TRANSIT\cite{bast-transit} uses a subset of vertices, called \emph{transit nodes}, and calculates all distances between these nodes.
			These vertices are selected in a way such that there are transit nodes on every shortest path.
			Determining a shortest path from the vertex $src$ to $trg$ with some transit nodes $T$ is fast because all distances between these four vertices are already known, which is used to determine the next vertex in the shortest path.
			Starting at the source $src$, the next vertex $v$ is the one for which $d(src, v) + d(v, trg) = d(src, trg)$ is true.
			These distances can quickly be calculated using the transit nodes of $T$ allowing this algorithm to find shortest paths in the order of microseconds.
		
		% TODO merge into "other" section above
