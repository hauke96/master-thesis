...
\label{sec:graph-routing}
	...
	\label{subsec:speedup-methods}
		...
		\subsubsection{Landmarks}
			...
			% TODO remove this math-part? Not really necessary ...
		...
		\subsubsection{Hub labels}
			...
			To not traverse through all labels to find $v$ from $u$, the second property (called \emph{cover property}) is very important:
			...
			Even though the requests can be answered very fast, actually faster than most other routing methods, it needs a lot of space and preprocessing time to store and determine all the labels.
...
\label{sec:geometric-routing}
	...
	\label{subsec:visibility-graph}
		...
		These alternatives do not always create such direct edges \cite{graser-osm-open-spaces}, which leads to detours and longer paths.
	...
	\label{subsec:continuous-dijkstra}
		...
		Such a tree might remind one of Dijkstra's algorithm, which creates such a shortest path tree as well.
		...
		Each wavefront can interact with edges and vertices but also with other wavefronts.
...
\section{Agent-based systems and simulations}
	...
	Many simulations use pedestrians (or generally humans) as agents to better understand human behaviors but many more scenarios with other types of agents can be simulated \cite{macal-introductory-tutorial}.
