...
%		The fact of potential suboptimal routes is discussed in the following.
		...
		\subsubsection{Valid angle areas}
			...
			Connecting lines, i.e. line segments connecting one convex hull part with another, always follow the clockwise or counter clockwise direction of the convex hull line segments as illustrated in \cref{fig:convex-hull}.
			If the convex hull part bends in counter clockwise direction, it means that the bearing decreases with each subsequent line segment and increases for clockwise hulls respectively.
			...
			if the destination vertex $v_2$ of the connecting line $(v_1, v_2)$ would be added to the obstacle of the source vertex (in this example $o_1$), then the connecting line would be part of the convex hull of this new enlarged obstacle.
...
%							
			...
			Allowing connecting lines against the turning direction of the source vertex would result in edges that will never be part of a shortest path.
			...
			This is due to the properties described at the beginning, that convex hulls themselves build shortest paths around obstacles, the overall shortest path is a sequence of convex hull parts, connecting lines connect these hulls following their turn direction and therefore are part of the overall shortest path.
			...
			This connecting line, which was created against the hull's turning direction, can be relaxed to a new connecting line that follows the turning direction.
			In \cref{fig:convex-hull}, the path $(v_0, v_1, v_3)$ with the connecting line $(v_1, v_3)$ can be relaxed to $(v_0, v_3)$ forming the actual shortest path.
			
			Following the idea of extending the source obstacle illustrates the contradiction that appear for breaking the turn direction criterion.
			Considering $o_1$ and extending it with the connecting line's destination vertex $v_3$ to $o'_1=o_1 \cup \{v_3\}$ yields the path $(s, v_0, v_3)$, which is the shortest path for the actual existing obstacles, because it follows the convex hull of the hypothetical obstacle $o'_1$.
			If the segment $(v_1, v_3)$ would be part of the shortest path from $s$ to $v_3$, it would not be part of the convex hull of $o'_1$ and therefore this segment cannot be part of the shortest path.
			
			Only connecting lines fulfilling the above described turning direction criterion need to be considered, which restricts all relevant vertices to certain angular ranges.
	...
	\label{subsec:step-4-graph merging}
		...
		\begin{figure}[h]
			...
			\caption{
				...
				\\
				\underline{Right:} After merging $r$. Two new vertices have been created and all edges are split on the intersection points. The graph size therefore increased by two nodes and eight edges (since all edges are bidirectional) to eight nodes and 14 edges.
			}
