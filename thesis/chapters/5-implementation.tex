...
%							\draw[blue,densely dashed,->] ($(v)+(0.075,-\d)$) -- ($(n3)+(-0.075,-\d)$);
...
%							\draw[red,densely dotted,->] ($(v)+(0.075,\d)$) -- ($(n3)+(-0.075,\d)$);
%							\draw[red,densely dotted,->] ($(v)+(\d,0.075)$) -- ($(n2)+(\d,-0.075)$);
							
							% Visibility edges to n2
							\tikzDot[Green]{(-2,1.3)}{vn6}
							\draw[Green,dashdotted,->] (v) -- (vn6);
%							\draw[Green,dashdotted,->] ($(v)+(-\d,0.075)$) -- ($(n2)+(-\d,-0.075)$);
%							\draw[Green,dashdotted,->] ($(v)+(-0.075,\d)$) -- ($(n1)+(0.075,\d)$);
	...
	\label{subsec:step-3-graph-creation}
		...
		It contains the final spatial graph along with all information needed to correctly add and connect new nodes, more precisely nodes for the start and target locations of a routing query.
	...
	\subsection{Step 4: Merging a road network into the visibility graph}
		...
		Effects on the routing performance are discussed in later sections. \todo{ref}
