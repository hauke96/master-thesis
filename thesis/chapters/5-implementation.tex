...
%							\draw[Green,dashdotted,->] ($(v)+(-0.075,\d)$) -- ($(n1)+(0.075,\d)$);
	...
	\label{subsec:step-3-graph-creation}
		...
		\todo{More details on the mappings?}
		...
		\begin{figure}[h]
			\begin{figcenter}
				\begin{tikzpicture}
					...
					\draw[<->,dotted] (v1n1) -- node[above right=0cm and 0.4cm] {$v$} (v1n2);
				...
				\begin{tikzpicture}
					...
					\draw[<->,dotted] (v1n1) -- node[above] {$v_1$} (i1);
					\draw[<->,dotted] (i1)   -- node[above] {$v_2$} (v1n2);
