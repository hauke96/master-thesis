...
\section{Geospatial data}
	...
	\label{subsec:data-structures}
		...
		\subsubsection{Standardization}
			...
			A community driven standardization strategy is used by the \term*{OpenStreetMap} (\term*{OSM}) project (covered in \cref{subsec:osm} with more details), where the projects Wiki defines certain keys and value but still explicitly allows arbitrary tags to exist\cite{osm-wiki-proposal-process}.
	...
	\label{subsec:file-formats}
		...
		\label{subsubsec:shapefile}
			...
			Even though the first byte of every geometry in the shapefile encodes its type \cite[4-23]{esri-shapefile-spec}, the standard allows only one type of geometry per shapefile \cite[4]{esri-shapefile-spec}.
		...
		\subsubsection{GeoTIFF}
			...
			Typical use cases for such images are aerial or satellite imagery and \term*{digital elevation models} (\term*{DEM}).
	...
	\subsection{OGC web services}
		...
		\subsubsection{WMS -- Web Map Service}
			...
			A \term*{Web Map Service} provides access to rendered images but not the the underlying raw data.
		...
		\subsubsection{WMTS -- Web Map Tile Service}
			...
			Similar to a WMS service, the \term*{Web Map Tile Service} specification was created to provide a more scalable service providing rendered map tiles\cite[12]{ogc-wmts}.
...
\label{sec:graph-routing}
	...
	\subsection{Routing engines}
		...
		Popular methods are \term{contraction hierarchies} or the use of \term{landmarks}, \term[transit node]{transit nodes} and \term[hub label]{hub labels}.
	...
	\label{subsec:speedup-methods}
		...
		\subsubsection{Hub labels}
			...
			A label must fulfill the following two properties \cite[11]{bast-transportation-networks}.
			...
			Even though the requests can be answered incredibly fast, actually faster than most other routing methods, it needs a lot of space and preprocessing time to store and determine all the labels \cite[23]{bast-transportation-networks}.
		...
		\subsubsection{Compressed path databases}
			...
			Uncompressed data is usually too large as the space requirement is in $\bigo{n^2}$ or even $\bigo{n^2 \log n}$ for graphs \cite[1]{botea-cpd-2013}.
...
\label{sec:geometric-routing}
	...
	\subsection{Visibility graphs}
		...
		Alternatives to the visibility method are based on Voronoi diagrams or skeletonization methods\cite[3-4]{graser-osm-open-spaces}.
		However, routing results on such graphs are not be optimal anymore, since a straight edge between visible vertices is the shortest possible connection and those alternatives so not always create such direct edges\cite[7]{graser-osm-open-spaces}.
	...
	\subsection{Continuous Dijkstra}
		...
		Due to this similarity, this strategy is also called the \term{continuous dijkstra} paradigm\cite[2]{mitchell-discrete-geodesic}.
		...
		The key difficulty of this approach is the efficient propagation of these wavefronts, which means the fast detection of collision with obstacles and other wavefronts\cite[3]{hershberger-suri}.
...
\section{Agent based systems and simulations}
	...
	Many simulations use pedestrians (or generally humans) as agents to better understand human behaviors but many more scenarios are thinkable \cite[8]{macal-introductory-tutorial}.
	...
	This not only includes \hyperref[sec:graph-routing]{graph based routing} but often utilizes \hyperref[sec:geometric-routing]{geometric routing} as well \cite[3]{kneidl-borrmann-hartmann-navigation}.
