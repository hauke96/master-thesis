...
\section{Geospatial data}
	...
	\subsection{Coordinate reference systems and projections}
		...
		The \term{coordinate system} defines, just like a mathematical coordinate system does, the extend and unit of coordinates.
		...
		An \term{ellipsoid} is a deformed sphere, which describes the Earth or a part of the Earth (for example one continent) as accurate as possible.
		...
		For two dimensional Cartesian coordinate systems, a \term*[map projection]{projection} is needed to turn each  spherical location on the ellipsoid to a two dimensional planar coordinate \cite[17]{ordenance-survey-booklet}.
		
		Next to the horizontal datum, a \term{vertical datum} is used to define the height of data \cite[9-10]{ordenance-survey-booklet}.
		This is done by defining a \term{geoid}, which can be another ellipsoid but that would result in very inaccurate data \cite[10]{ordenance-survey-booklet}.
		...
		For visualization purposes, a \term{map projection} might be needed to turn coordinates of a specific CRS into to coordinates of the map.
		The map can be a two dimensional printed or digital map but digital maps can also be on a three dimensional ellipsoid.
		Therefore, the map projection can be a different projection as used in a two dimensional Cartesian CRS and therefore transformations between different projections are necessary.
		...
		Each entry in this database can be identified by a code with the prefix \texttt{EPSG:} followed by a numeric code, for example \texttt{EPSG:4326} for the global and widely used WGS 84.
