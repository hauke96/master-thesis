	...
	\subsection{OSM-based datasets}
		...
		\subsubsection{Dataset without roads or obstacles}
			...
			\begin{figure}[h!]
				...
				\label{fig:eval-import-osm-no-roads-obstacles}
			\end{figure}
			...
			Considering a dataset without roads, the kNN search is completely unaffected by this fact on both, rural and city categories.
			Only the time of the road merge operation is directly affected since, except building passages, no roads exist to merge into the graph.
			
			\Cref{fig:eval-import-osm-no-roads-obstacles-city} (results of \enquote{OSM city} datasets) shows that without roads the merge operation took longer compared to the dataset without obstacles.
			This is due to the 165 remaining building passages still existing in the dataset without roads.
			Even though the processing still takes some time, the merge operation in the \enquote{no roads} variant of the \enquote{OSM city} dataset only required 5.14\% and using the \enquote{no obstacles} only 0.33\% of the time compared to the normal dataset.
			
			\Cref{fig:eval-import-osm-no-roads-obstacles-rural} (results of \enquote{OSM rural} datasets) shows the expected results for the merge operation, meaning the time decreased from 12.9s to 1.47ms, since no building passages exist in the rural datasets.
			
			Apart these different results, both dataset categories show similar results for the kNN search, graph generation and overal processing time.
			
			\begin{table}
				\begin{center}
					\begin{tabularx}{\textwidth}{p{3cm}|X|XX|XX}
...
Total time			& 963.1		& 535.29	& 44.42\%						& 1.43			& 99.85\%
					\end{tabularx}
				\end{center}
				\caption{Results of the \enquote{OSM city} datasets in absolute time (in seconds) and relative decrease in time compared to the normal dataset.}
				\begin{center}
					\begin{tabularx}{\textwidth}{p{3cm}|X|XX|XX}
Operation			& Normal 	& No roads	& Decrease compared to normal	& No obstacles	& Decrease compared to normal	\\
\hline
...
Total time			& 21,658	& 8709		& 59.79\%						& 39.7			& 99.82\%
					\end{tabularx}
				\end{center}
				\caption{Results of the \enquote{OSM rural} datasets in absolute time (in ms) and relative decrease in time compared to the normal dataset.}
			\end{table}
