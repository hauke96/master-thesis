...
\section{Route correctness and quality}
	...
	\todo{Describe weighting function}
	...
	\subsection{Usefulness of routes}
		...
		The usefulness of the resulting routes are evaluated in two criterion.
		First, a manual analysis is performed comparing the route calculated using the hybrid routing algorithm with the expected path a real human would likely choose and also with the path a classical routing algorithms would determine.
		Second, factors are determined on how much shorter a route is compared to classical routing algorithms and also how much longer the route is compared to the optimal euclidean beeline distance.
		...
		\subsubsection{Manual route analysis}
			...
			First, the quality of a route significantly decreases with missing or wrong data.
			Second, within regions of dense buildings and road networks, the expected route, routes determined by graph-based routing and routes determined by the hybrid routing algorithm follow these roads very often.
			Third, large roads create wide areas without any obstacles, which might result in unrealistic road crossings.
			This can be seen in \Cref{fig:eval-city-road-crossing} with crossings in a six-lane road, which would be dangerous and unrealistic for real-world pedestrians.
			...
			% Mark segments in agent trips that are not walkable in real world
		
		% How much shorter are the routes?
		\subsubsection{Route length analysis}